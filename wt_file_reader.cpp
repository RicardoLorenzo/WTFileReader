...
void WTFileReader::mapFile() {
    ...
    wt_r->map =
        mmap((caddr_t)0, wt_r->f_size, PROT_READ, MAP_PRIVATE | MAP_NORESERVE, wt_r->f_desc, 0);
...
int WTFileReader::readBlockDesc() {
     ...
     */
    wt_r->c_offset = 0;
    memcpy(wt_r->f_block_desc,
           (static_cast<unsigned char*>(wt_r->map) + wt_r->c_offset),
           WT_BLOCK_DESC_SIZE);
...
int WTFileReader::readHeaders(wt_page_t* page) {
     ...
     */
    memcpy(page->page_header,
           (static_cast<uint8_t*>(wt_r->map) + wt_r->c_offset),
           WT_PAGE_HEADER_SIZE);
    memcpy(page->block_header,
           (static_cast<uint8_t*>(wt_r->map) + (wt_r->c_offset + WT_PAGE_HEADER_SIZE)),
