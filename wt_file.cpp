...
void WTFile::printPage(wt_page_t* page, wt_cell_unpack_t* entries) {
    ...
    if (page->page_header->type == WT_PAGE_ROW_LEAF) {
        for (int i = 0; i < page->page_header->entries; i++) {
             ...
             */
            std::cout << "    cell desc: 0x" << std::hex << (unsigned int)(cell.desc)
                      << " data: <sz=" << std::dec << cell.size << " v=[0x" << std::hex
                      << (unsigned int)cell.data[0] << " ... 0x" << std::hex
                      << (unsigned int)cell.data[cell.size - 1] << "]>" << std::endl;
        }
    }
}
